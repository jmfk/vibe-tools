...
def test_check_connection_failure():
    with patch("socket.create_connection", side_effect=ConnectionRefusedError):
        assert (
            check_connection("postgres", {"host": "localhost", "port": 5432}) is False
        )
...
def test_setup_test_command(runner, mock_config, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "vibe_tools.utils.GLOBAL_CONFIG_FILE", tmp_path / "global_config.json"
    )
...
def test_setup_service_commands_save_config(runner, tmp_path, command, service_key):
    ...
    with patch("vibe_tools.utils.CONFIG_FILE", config_file):
        with patch("vibe_tools.setup.detect_docker_service", return_value=detection):
            with patch(
                "vibe_tools.setup.click.prompt", side_effect=_prompt_return_default
            ):
