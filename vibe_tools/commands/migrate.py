...
def run_reconciliation(quiet=False):
    ...
    if PRODUCT_DIR.exists():
        for yaml_file in PRODUCT_DIR.glob("*.yaml"):
            ...
            if not target_path.exists():
                if not quiet: click.echo(f"  📦 Moving {yaml_file.name} to implementation/prds/")
                shutil.move(str(yaml_file), str(target_path))
            else:
                if not quiet: click.echo(f"  ⚠️  {yaml_file.name} already exists in implementation/prds/, removing from product/")
                yaml_file.unlink()
    ...
    if VIBE_PROJECT_DIR.exists():
        for md_file in VIBE_PROJECT_DIR.rglob("*.md"):
            ...
            if not target_path.exists():
                if not quiet: click.echo(f"  📦 Moving {md_file} to product/")
                shutil.move(str(md_file), str(target_path))
            else:
                if not quiet: click.echo(f"  ⚠️  {md_file.name} already exists in product/, removing from implementation/")
                md_file.unlink()
    ...
    for filename, initial_content in core_md_files.items():
        ...
        if not file_path.exists():
            ...
            if (PRD_DIR / yaml_name).exists() or filename in ["build.md", "testing.md"]:
                if not quiet: click.echo(f"  📄 Creating missing core spec: {filename}")
    ...
    if not quiet: click.echo("📁 Reconciling PRD file locations with state.json...")
    ...
    for prd_id, path in found_yamls.items():
        ...
        if is_completed and prd_id not in completed_prds:
            if not quiet: click.echo(f"  ✨ Found {prd_id} in history, marking as completed.")
            state.setdefault("completed_prds", []).append(prd_id)
            state_changed = True
        elif not is_completed and prd_id in completed_prds:
            if not quiet: click.echo(f"  ⚠️  {prd_id} found in backlog but marked completed in state. Moving to history.")
        ...
        if prd_id not in state.get("plans", {}):
            if not quiet: click.echo(f"  ➕ Adding missing PRD {prd_id} to plans.")
    ...
    if not quiet: click.echo("🌿 Checking for active feature branches...")
        ...
        for line in stdout.strip().splitlines():
            ...
            if prd_id in state.get("plans", {}) and state["plans"][prd_id].get("status") == "pending":
                if prd_id not in started_prds:
                    if not quiet and click.confirm(f"  💡 Found active branch '{branch}'. Mark {prd_id} as IN_PROGRESS?"):
                        state.setdefault("started_prds", []).append(prd_id)
                        state["plans"][prd_id]["status"] = "in_progress"
                        state_changed = True

    # 4. Old-style migration (cleanup)
    # Migrate from implementation/prds/
    ...
    if old_prd_dir.exists() and old_prd_dir.is_dir():
        for yaml_file in old_prd_dir.glob("*.yaml"):
            ...
            if not target_path.exists():
                if not quiet: click.echo(f"  📦 Moving legacy {yaml_file.name} to {target_dir.name}/")
                shutil.move(str(yaml_file), str(target_path))
                state_changed = True
            elif yaml_file.parent == old_prd_dir:
                yaml_file.unlink()

    # 5. Migrate from product/
    if PRODUCT_DIR.exists() and PRODUCT_DIR.is_dir():
        for md_file in PRODUCT_DIR.glob("*.md"):
            ...
            if not target_path.exists():
                if not quiet: click.echo(f"  Moving {md_file} to {target_dir}/")
                shutil.copy2(md_file, target_path)
                md_file.unlink()
            else:
                if md_file.parent == PRODUCT_DIR:
                    if not quiet: click.echo(f"  Skipping {md_file}, already exists in {target_dir}/")

    # 6. Reconcile Issue Index
    if not quiet: click.echo("📋 Reconciling issue index...")
    from vibe_tools.issues import load_all_issues, save_issue
    all_issues = load_all_issues()
    if all_issues:
        for issue in all_issues:
            save_issue(issue)
        if not quiet: click.echo(f"  ✨ Re-indexed {len(all_issues)} issues.")

    # 7. Fix paths inside moved files
    if not quiet: click.echo("📝 Fixing paths inside implementation and product files...")
    _fix_content_paths(quiet=quiet)

    # 8. Cleanup legacy directories
    _cleanup_legacy_dirs(quiet=quiet)
...
def _fix_content_paths(quiet=False):
    ...
    for target_dir in target_dirs:
        ...
        if not quiet: click.echo(f"  🔍 Fixing paths in {target_dir}/...")
        for file_path in target_dir.rglob("*"):
            ...
            try:
                ...
                if new_content != content:
                    if not quiet: click.echo(f"    ✨ Updated {file_path}")
                    file_path.write_text(new_content)
            except (UnicodeDecodeError, PermissionError):
                continue

def _cleanup_legacy_dirs(quiet=False):
    ...
    for d in legacy_dirs:
        ...
        if path.exists() and path.is_dir():
            ...
            if not contents:
                if not quiet: click.echo(f"  🧹 Removing empty legacy directory: {d}")
                try:
                    path.rmdir()
                except OSError as e:
                    if not quiet: click.echo(f"  ⚠️ Could not remove {d}: {e}")
            else:
                if len(contents) == 1 and contents[0].name == ".DS_Store":
                    contents[0].unlink()
                    if not quiet: click.echo(f"  🧹 Removing empty legacy directory (after cleaning .DS_Store): {d}")
                    try:
                        path.rmdir()
                    except OSError as e:
                        if not quiet: click.echo(f"  ⚠️ Could not remove {d}: {e}")
