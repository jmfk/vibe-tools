...
def investigate_impl(ctx, query=None):
    """Internal implementation of investigation."""
    ...
    title = query or click.prompt("\nIssue Title")
    severity = click.prompt(
        "Severity",
        type=click.Choice(["low", "medium", "high", "critical"]),
        default="medium",
    )
    service = click.prompt("Service", default="core")
    ...
    content += (
        f"## Investigation Notes\n- Created via `vibe plan investigate` on {now}\n"
    )
...
def register_investigate(cli):
    ...
    @click.pass_context
    def investigate_command(ctx, logs, service, github):
        """Create issues via guided investigation."""
        investigate_impl(ctx)
