...
def _check_and_suggest_dependencies(prd: PRD, all_prds: List[PRD], completed: List[str]):
    ...
    for dep_id in missing:
        ...
        if not dep_prd:
            console.print(f"[red]❌ Dependency {dep_id} not found in any PRD files.[/red]")
            continue
        ...
        if status == "done":
            ...
            if click.confirm(
                ...
                if _check_and_suggest_dependencies(dep_prd, all_prds, completed):
                    ...
                    dep_prd.path.unlink()
                    console.print(f"[green]✅ Started {dep_id}. Run 'vibe implement' to begin.[/green]")
                    return False
        else:
            console.print(
...
def register_prd(cli):
    ...
    @click.option("--filter", "filter_text", default="", help="Filter PRDs by title or ID.")
    def plan_prds(page_size, filter_text):
        ...
        def is_filtering(): return state["filter_mode"]
        def is_not_filtering(): return not state["filter_mode"]
        ...
        def refresh_files():
            ...
            if f_text:
                def matches(path):
                    if f_text in path.name.lower(): return True
                    try:
                        p = load_prd(path)
            ...
            for i in [0, 1]:
                ...
                if not files:
                    ...
                    state[f"pane_{i}_selected"] = min(state[f"pane_{i}_selected"], len(files) - 1)
                    state[f"pane_{i}_page"] = state[f"pane_{i}_selected"] // page_size

        def render_pane(pane_idx: int, files: List[pathlib.Path], selected_idx: int, page: int):
            is_active = (state["active_pane"] == pane_idx) and not state["dep_select_mode"] and not state["filter_mode"]
            title = "NEXT (Implementation Queue)" if pane_idx == 0 else "PLANNING (Inbox + Backlog)"
            ...
            for i, f in enumerate(page_files):
                ...
                try:
                    p = load_prd(f)
                    row_style = Style(reverse=True) if (is_selected and is_active) else Style()
                    if is_selected and not is_active and not state["dep_select_mode"] and not state["filter_mode"]: 
                        row_style += Style(bgcolor="grey23")
                    
                    id_text = Text(p.id, style="cyan" if p.status == "inbox" else "white")
                    deps_text = ", ".join(p.depends_on) if p.depends_on else "-"
                    table.add_row(str(abs_idx + 1), id_text, p.title, deps_text, style=row_style)
                except Exception:
                    table.add_row(str(abs_idx + 1), "-", f.name, "-", style="red")

            footer = f"Page {page + 1}/{max(1, total_pages)} ({len(files)} items)"
            return Panel(table, title=title, subtitle=footer, border_style=border_style, expand=True)
        ...
        def render_dep_selection():
            ...
            for i, f in enumerate(page_files):
                ...
                try:
                    ...
                    if f.parent.resolve() == PLANNING_INBOX_DIR.resolve(): s_col = "cyan"
                    elif f.parent.resolve() == PRODUCT_NEXT_DIR.resolve(): s_col = "magenta"
                    else: s_col = "white"
                    table.add_row(str(abs_idx + 1), p.id, Text(p.status.upper(), style=s_col), p.title, style=row_style)
                except Exception:
                    table.add_row(str(abs_idx + 1), "-", "ERROR", f.name, style="red")

            footer = f"Page {page + 1}/{max(1, total_pages)} | ENTER: Select | ESC: Cancel"
            return Panel(table, title="SELECT DEPENDENCY (All PRDs)", subtitle=footer, border_style="bold yellow", expand=True)

        def render_all():
            refresh_files()
            
            pane_0 = render_pane(0, state["pane_0_files"], state["pane_0_selected"], state["pane_0_page"])
            pane_1 = render_pane(1, state["pane_1_files"], state["pane_1_selected"], state["pane_1_page"])
            ...
            if state["filter_mode"]:
                ...
                output.append(Text(f"\n🔍 Active Filter: '{state['current_filter']}' (S-F to clear)", style="cyan"))
        ...
        @kb.add("tab")
        def _(event):
            if state["dep_select_mode"] or state["filter_mode"]: return
            state["active_pane"] = 1 - state["active_pane"]

        @kb.add("up")
        def _(event):
            if state["filter_mode"]: return
            if state["dep_select_mode"]:
        ...
        def _(event):
            if state["filter_mode"]: return
            if state["dep_select_mode"]:
                state["dep_selected_idx"] = min(len(state["dep_files"]) - 1, state["dep_selected_idx"] + 1)
                state["dep_page"] = state["dep_selected_idx"] // page_size
            else:
                p = state["active_pane"]
                files = state[f"pane_{p}_files"]
                state[f"pane_{p}_selected"] = min(len(files) - 1, state[f"pane_{p}_selected"] + 1)
                state[f"pane_{p}_page"] = state[f"pane_{p}_selected"] // page_size
        ...
        def _(event):
            ...
            state[f"pane_{p}_page"] = min(max(0, total_pages - 1), state[f"pane_{p}_page"] + 1)
            state[f"pane_{p}_selected"] = state[f"pane_{p}_page"] * page_size

        @kb.add("f", filter=Condition(is_not_filtering) & ~Condition(lambda: state["dep_select_mode"]))
        def _(event):
            ...
            if state["dep_select_mode"]:
                ...
                try:
                    ...
                    if dep_prd.id == target_prd.id: state["message"] = "❌ A PRD cannot depend on itself."
                        ...
                        target_prd.save()
                        state["message"] = f"✅ Added dependency: {target_prd.id} -> {dep_prd.id}"
                except Exception as e: state["message"] = f"❌ Error: {str(e)}"
                state["dep_select_mode"] = False
        ...
        def _(event):
            ...
            try:
                ...
                if p_idx == 1:
                    prd.status = "planned"; target_path = PRODUCT_NEXT_DIR / selected_file.name
                    prd.save(target_path); selected_file.unlink()
                    state["message"] = f"✅ Moved {prd.id} to NEXT."
                else:
                    prd.status = "backlog"; target_path = PRODUCT_BACKLOG_DIR / selected_file.name
                    prd.save(target_path); selected_file.unlink()
        ...
        def _(event):
            ...
            selected_file = files[sel_idx]
            try:
                prd = load_prd(selected_file); prd.status = "rejected"
                prd.save(PLANNING_REJECTED_DIR / selected_file.name); selected_file.unlink()
        ...
        def _(event):
            ...
            selected_file = files[sel_idx]
            try:
                prd = load_prd(selected_file); prd.status = "inbox"
                prd.save(PLANNING_INBOX_DIR / selected_file.name)
                if selected_file != (PLANNING_INBOX_DIR / selected_file.name): selected_file.unlink()
        ...
        def _(event):
            ...
            selected_file = files[sel_idx]
            try:
                prd = load_prd(selected_file); prd.status = "backlog"
                prd.save(PRODUCT_BACKLOG_DIR / selected_file.name)
                if selected_file != (PRODUCT_BACKLOG_DIR / selected_file.name): selected_file.unlink()
                state["message"] = f"📦 Moved {prd.id} to BACKLOG."
            except Exception as e: state["message"] = f"❌ Error: {str(e)}"
        ...
        def _(event):
            if state["dep_select_mode"] or state["filter_mode"]: return
            p_idx = state["active_pane"]; files = state[f"pane_{p_idx}_files"]
            if not files: return
            ...
            state["dep_selected_idx"] = 0; state["dep_page"] = 0

        @kb.add("escape")
        def _(event):
            if state["dep_select_mode"]: state["dep_select_mode"] = False
            if state["filter_mode"]: state["filter_mode"] = False

        @kb.add("q")
        @kb.add("c-c")
        def _(event):
            if state["dep_select_mode"]: state["dep_select_mode"] = False
            elif state["filter_mode"]: state["filter_mode"] = False
            else: event.app.exit()
    ...
    def stop_prd():
        ...
        for f in in_progress:
            ...
            f.unlink()
            console.print(f"[green]✅ Stopped {p.id} and moved back to backlog.[/green]")
    ...
    @click.argument("target", type=click.Choice(["backlog", "history", "rejected", "in_progress", "next", "inbox"]))
    def move_prd(prd_id, target):
        ...
        target_map = {
            "backlog": PRODUCT_BACKLOG_DIR, "history": PRODUCT_HISTORY_DIR,
            "rejected": PLANNING_REJECTED_DIR, "in_progress": PRODUCT_IN_PROGRESS_DIR,
            "next": PRODUCT_NEXT_DIR, "inbox": PLANNING_INBOX_DIR,
        }

        target_dir = target_map[target]
        ensure_dir(target_dir)
        p = load_prd(found_path)
        p.status = {"history": "done", "in_progress": "in_progress", "next": "planned", "inbox": "inbox"}.get(target, "backlog")
