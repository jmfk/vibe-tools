...
def register_normalize(cli):
    @click.command()
    @click.argument("input_file", type=click.Path(exists=True, path_type=pathlib.Path), required=False)
    @click.option("--auto-overwrite", "--yes", "-y", is_flag=True, help="Automatically overwrite existing YAML files.")
    @click.pass_context
