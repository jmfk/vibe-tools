...
from vibe_tools.utils import get_agent_command, logger, run_agent, get_prompt, log_issue
...
def run_test_fix_loop(agent="cursor-agent", caffeinate=False, fast=False, stream=False):
    ...
    try:
        ...
        logger.warning("\n🛑 Test and Fix loop interrupted by user.")
        logger.info("Progress has been saved. Run 'vibe test-fix' again to resume.")
        return False
