    ...
    <div className="flex flex-col h-full overflow-hidden">
      <PanelGroup orientation="vertical">
        <Panel defaultSize={50} minSize={20}>
          <div className="flex flex-col h-full overflow-hidden border-b border-border/50">
            <div className="p-3 border-b border-border/50 flex items-center justify-between bg-panel/30 shrink-0">
              <div className="flex items-center gap-2">
                <Files size={14} className="text-muted" />
                <span className="text-[10px] font-bold uppercase tracking-widest text-foreground">Open Documents</span>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-1 space-y-1 scrollbar-none">
              ...
              {openArtifacts.length === 0 && (
                <div className="py-8 text-center h-full flex flex-col items-center justify-center">
                  <Files size={24} className="mb-2 opacity-10" />
        ...
        <PanelResizeHandle className="h-1.5 bg-transparent hover:bg-accent/10 active:bg-accent/20 transition-all duration-150" />

        <Panel defaultSize={50} minSize={20}>
          <div className="flex flex-col h-full overflow-hidden">
            <div className="p-3 border-b border-border/50 flex items-center gap-2 bg-panel/30 shrink-0">
              <Tag size={14} className="text-muted" />
              <span className="text-[10px] font-bold uppercase tracking-widest text-foreground">Properties</span>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 scrollbar-none">
              {selectedArtifact ? (
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="text-xs">
                      <span className="px-1.5 py-0.5 rounded border text-[10px] font-bold bg-panel border-border" style={{ borderColor: accentColor + '40', color: accentColor }}>
                  ...
                  <div>
                    ...
                    <div className="text-[10px] font-mono break-all leading-tight text-muted opacity-80">{selectedArtifact.relPath}</div>
                  </div>
                  {selectedArtifact.id && (
                    <div>
                      <div className="text-[9px] font-bold uppercase tracking-widest mb-1 text-muted">Identifier</div>
                      <div className="text-[10px] font-mono text-accent" style={{ color: accentColor }}>{selectedArtifact.id}</div>
                    </div>
                  )}
                </div>
              ) : (
                <div className="h-full flex flex-col items-center justify-center text-center opacity-40">
                  ...
                  <p className="text-[10px] italic">No item selected</p>
                </div>
              )}
            </div>
          </div>
