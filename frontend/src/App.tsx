      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          <div className="bg-zinc-800/50 rounded-lg p-3 border border-zinc-700/50">
            <div className="text-xs font-bold text-blue-400 mb-1 uppercase tracking-wider">Architect</div>
            <div className="text-sm prose prose-invert max-w-none">
              <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeHighlight]}>
                Hello! I am the Architect agent. I've initialized the Tauri Dashboard Core. You can now explore files, monitor logs, and run commands from this interface.
              </ReactMarkdown>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-zinc-800 bg-zinc-900">
          <div className="relative">
            <input 
              ...
              className="w-full bg-zinc-800 border border-zinc-700 rounded-md py-2 px-3 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 text-zinc-100"
            />
          </div>
        </div>
      </div>
        ...
        <div className="p-4 space-y-6 overflow-y-auto">
          ...
          <section>
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-green-500" />
                <span className="text-zinc-300">Vite Dev Server</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-zinc-600" />
                <span className="text-zinc-500">Log Streamer</span>
              </div>
            </div>
          </section>
          ...
          <section>
            ...
            <div className="bg-zinc-800/30 rounded p-3 border border-zinc-800">
              <div className="text-2xl font-bold text-zinc-100">$0.42</div>
              <div className="text-[10px] text-zinc-500 mt-1">Today's estimated usage</div>
            </div>
          </section>
        </div>
      </div>
  ...
  <div className="h-full flex flex-col space-y-4">
    <div className="flex items-center justify-between">
      ...
      <div className="flex gap-2">
        <button className="px-2 py-1 bg-zinc-800 rounded text-xs hover:bg-zinc-700">Clear</button>
        <button className="px-2 py-1 bg-zinc-800 rounded text-xs hover:bg-zinc-700">Pause</button>
      </div>
    </div>
    <div className="flex-1 bg-black border border-zinc-800 rounded-lg font-mono text-sm p-4 overflow-y-auto">
      <div className="text-zinc-500">[2026-01-14 10:00:01] INFO: Initializing vibe-tools...</div>
      <div className="text-zinc-500">[2026-01-14 10:00:02] INFO: Loading configuration from config.json</div>
      <div className="text-green-500">[2026-01-14 10:00:03] SUCCESS: Backend connected successfully</div>
      <div className="text-zinc-500">[2026-01-14 10:00:05] DEBUG: Watcher started on product/</div>
      <div className="text-yellow-500">[2026-01-14 10:00:10] WARN: High memory usage detected in worker-1</div>
...
const RunnerView = () => (
  <div className="space-y-6">
    ...
    <div className="space-y-4">
      {[
        { name: 'npm run dev', description: 'Start development servers' },
        { name: 'make test', description: 'Run all project tests' },
        { name: 'python setup.py install', description: 'Install project dependencies' }
      ].map((cmd) => (
        <div key={cmd.name} className="flex items-center justify-between p-4 bg-zinc-900 border border-zinc-800 rounded-lg">
          <div>
          ...
          <button className="flex items-center gap-2 px-4 py-2 bg-zinc-100 text-zinc-950 rounded-md font-semibold text-sm hover:bg-white transition-colors">
            <PlayCircle size={16} />
            Run
          </button>
        </div>
